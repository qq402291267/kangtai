...
unsigned char  AES_iv[] = "abcdef1234567890";  /* align */
//unsigned char  local_key[16] = "0123456789abcdef";
unsigned char dev_tls_tmp_data[256];

int device_aes_encrypt(unsigned char * key,unsigned char  *data, int len)
{
    unsigned char  i;
	Aes enc;
	int dat_len=len;
	AesSetKey(&enc, key, AES_BLOCK_SIZE, key, AES_ENCRYPTION);
    ...
    if((dat_len-len)>0)
    {
        for(i=len;i<dat_len;i++)
        {
            data[i]=dat_len-len;
        }
    }
    else if((dat_len-len)==0)
    {
        dat_len=dat_len+0x10;
        for(i=len;i<dat_len+16;i++)
        {
            data[i]=0x10;
        }
    }
    
	AesCbcEncrypt(&enc, (byte *)dev_tls_tmp_data, data, dat_len);
	memcpy(data,dev_tls_tmp_data, dat_len);
	return dat_len;
}


unsigned char device_aes_decrypt(unsigned char * key,unsigned char *data, int len)
{
	int dat_len= len;
	Aes dec;
	AesSetKey(&dec, key, AES_BLOCK_SIZE, key, AES_DECRYPTION);
	AesCbcDecrypt(&dec, (byte *)dev_tls_tmp_data, data, dat_len);
	dat_len=dat_len-dev_tls_tmp_data[dat_len-1];
	memcpy(data,dev_tls_tmp_data, dat_len);
	return dat_len;
}
