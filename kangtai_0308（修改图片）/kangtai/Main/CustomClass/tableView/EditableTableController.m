...
- (instancetype)initWithTableView:(UITableView *)tableView
{
    ...
    NSAssert(tableView.rowHeight > 0, @"The tableView's rowHeight must be set.");
...
- (void)didRecognizeLongPress:(UILongPressGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan)
    {
        ...
        if (iOS_version < 7.0) {
            self.snapshotView = [[UIView alloc] initWithFrame:cell.frame];
            UIGraphicsBeginImageContextWithOptions (cell.bounds.size, YES, 1) ;
            [cell.layer renderInContext :UIGraphicsGetCurrentContext( )] ;
            UIImage *viewImage = UIGraphicsGetImageFromCurrentImageContext( ) ;
            UIGraphicsEndImageContext( ) ;
            UIImageView *shot = [ [ UIImageView alloc ] initWithImage:viewImage] ;
            [self.snapshotView addSubview :shot];
        } else {
