...
- (void)initView {
    ...
    self.ratioView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, 300)];
    self.ratioView.center = CGPointMake([UIScreen mainScreen].bounds.size.width / 2, [UIScreen mainScreen].bounds.size.height / 2);
...
-(UIImage *)getSubImage{
    ...
    if (self.latestFrame.size.width < self.ratioView.frame.size.width) {
        CGFloat newW = self.originalImage.size.width;
        CGFloat newH = newW * (self.ratioView.frame.size.height / self.ratioView.frame.size.width);
        x = 0; y = y + (h - newH) / 2;
        w = newH; h = newH;
    }
    if (self.latestFrame.size.height < self.ratioView.frame.size.height) {
        CGFloat newH = self.originalImage.size.height;
        CGFloat newW = newH * (self.ratioView.frame.size.width / self.ratioView.frame.size.height);
        x = x + (w - newW) / 2; y = 0;
        w = newH; h = newH;
    }
