...
+(NSString *)getLunarHoliDayDate:(NSDate *)date{  
    ...
    NSString *key_str = [NSString stringWithFormat:@"%d-%ld",(int)localeComp.month,(long)localeComp.day];
...
+(NSString *)getLunarSpecialDate:(NSDate *)date  {   
    ...
    int array_index = (int)(([self getYear:date] - START_YEAR)*12+[self getMonth:date]) -1 ;
    ...
    if([self getDay:date] == day){   
        index = (int)(([self getMonth:date]-1) *2 + ([self getDay:date]>15? 1: 0));
    }  
...
{
    return [self dateAfterDay:(int)-[self getDay:date] + 1:date];
}
//该月的最后一天
