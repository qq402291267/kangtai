...
- (void)updateTimeLabel
{
    ...
    if ([cmp1 day] == [cmp2 day]) { // 今天
        formatter.dateFormat = @"Present day HH:mm";
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@：%@",NSLocalizedString(@"Last Updated", nil), time];
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state) {
        ...
        {
            // 设置文字
			self.statusLabel.text = NSLocalizedString(@"Refresh", nil);
